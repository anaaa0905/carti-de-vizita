...
const carteUnica = document.getElementById('carte-unica');
if (carteUnica) {
  fetch('https://sheetdb.io/api/v1/3fvnik555at2l')
    ...
    .then(data => {
      ...
      if (isNaN(idx) || !echipa[idx]) {
        carteUnica.innerHTML = '<p style="color:red;font-size:22px;">Angajat inexistent!</p>';
        return;
      }
      carteUnica.innerHTML = carteVizitaHTML(echipa[idx], idx);
      ...
      try {
        ...
        if (qrDiv) qrDiv.innerHTML = '<span style="color:red;font-size:10px;">VCard prea mare pentru QR</span>';
      }
      setTimeout(() => {
